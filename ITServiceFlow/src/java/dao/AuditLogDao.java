...
public class AuditLogDao extends DbContext {
    ...
    public List<AuditLog> getLogs(String userName, String action, java.sql.Date from, java.sql.Date to, int offset,
            int limit) {
    ...
    public int getTotalLogs(String userName, String action, java.sql.Date from, java.sql.Date to) {
        int count = 0;
        StringBuilder sql = new StringBuilder(
    ...
    public List<String> getDistinctActions() {
        ...
        String sql = "SELECT DISTINCT Action FROM AuditLogs ORDER BY Action";
        try {
            PreparedStatement stm = connection.prepareStatement(sql);
            java.sql.ResultSet rs = stm.executeQuery();
            while (rs.next()) {
